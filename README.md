...
用户所说的其他对话可以指 Codex 或 ChatGPT 对话，但 `0.1` 不接受对话导出或上传，也不假设 Skill 可以无条件搜索全部历史对话。只有当运行环境明确提供某个历史对话的上下文时，Agent 才能使用它；否则必须说明无法读取，不能伪装成已经读取。
